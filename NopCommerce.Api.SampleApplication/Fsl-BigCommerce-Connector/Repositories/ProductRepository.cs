...
namespace Fsl.BigCommerce.Api.Connector.Repositories
{
    public sealed class ProductRepository
    {
        ...
        public async Task<ProductCollectionResponse> GetAll()
        {
            ...
            BigCommerceAPIClient g = new BigCommerceAPIClient("zzr434k017");


            var data = await g.Client.GetProductsAsync(); // Get<ProductsRootObject>(jsonUrl);
        ...
        public async Task<object> SyncBigCommerceProductToHubspot(ProductCollectionResponse productsRootObject)
        {
            ...
            var products = productsRootObject.Data.Where(product => !string.IsNullOrEmpty(product.Name));
            ...
            {
                var onlineVisible = product.ABCCode?.value?.ToString();
                if (onlineVisible != null && onlineVisible.Equals("W"))
                {
                    decimal msrp = 0;
                    var msrpstring = product.MSRP?.value.ToString();
                    if (!string.IsNullOrEmpty(msrpstring))
                    {
                        msrp = decimal.Parse(msrpstring);
                    }
                    var content = Convert.ToString(product.Content?.value);
                    var description = product.Description?.value.ToString();
                    var inventoryId = product.InventoryID?.value.ToString();
                    //var itemStatus = stockItem.ItemStatus?.value.ToString();
                    var imageUrl = product.ImageURL?.value.ToString();
